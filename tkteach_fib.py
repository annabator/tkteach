...
class tkteach:
	...
	def initializeCategories(self):
		...
		for keyi in self.categories:
	        # only 1 char categories allowed now, no brackets or longer labels
			if keyi[0] in self.keyBindings:
				...
				self.keyBindings.append(keyi[0].lower())
			#else:
			#	print(keyi)
			#	print('Wrong categories.')
			#	exit()
		print('Key bindings: ' + str(self.keyBindings))
	...
	def loadImage(self):
		...
		# ignore DS store!!
		# this code doesn't work tho so just let it throw error, it just skips over the bad file anyway!
		# if self.imageListDir[self.imageSelection] == './ds/'+ DIR + '/.DS_Store':
